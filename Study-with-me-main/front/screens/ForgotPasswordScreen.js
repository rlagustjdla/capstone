...
export default function ForgotPasswordScreen() {
  ...
  const handleRequestCode = async () => {
    try {
      await axios.post('http://112.162.196.250:3000/auth/request-reset', { email });
  ...
  try {
    ...
    // ✅ 서버에 코드 검증 요청
    const response = await axios.post('http://112.162.196.250:3000/auth/verify-reset-code', {
      email,
      code: resetCode
    });
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await axios.post('http://112.162.196.250:3000/auth/reset-password', {
        email,
        code: resetCode,
        newPassword
      });
